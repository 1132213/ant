...
class FeatureExtractor:
    ...
    def encode_board(self, state: BackendState, player: int) -> np.ndarray:
        enemy = 1 - player
        # 将通道数扩展到 35，为战斗蚂蚁、闪电风暴以及护盾层数等动态特征提供独立通道
        board = np.zeros((35, MAP_SIZE, MAP_SIZE), dtype=np.float32)
        ...
        for tower in state.towers:
            ...
            board[32, tower.x, tower.y] = tower.hp / max(getattr(tower, 'max_hp', 15.0), 1.0)
            
        for ant in state.ants:
            # 区分战斗蚂蚁和普通蚂蚁
            if getattr(ant, 'kind', 0) == 1:
                channel = 28 if ant.player == player else 29
                if ant.hp < ant.max_hp / 2:
                    board[34, ant.x, ant.y] = 1.0 # 自爆标记
            else:
            ...
            board[33, ant.x, ant.y] += getattr(ant, 'evasion_layers', 0) / 3.0 # 紧急回避护盾层数
            if ant.frozen:
        ...
        for effect in state.active_effects:
            ...
            # 对于闪电风暴，增加一个专属的倒计时通道，用于提示拆塔节点
            storm_damage_channel = None
            if effect.weapon_type == SuperWeaponType.LIGHTNING_STORM:
                storm_damage_channel = 30 if effect.player == player else 31
                # 计算距离下一个 5 的倍数回合还有几回合
                turns_to_damage = effect.remaining_turns % 5
                # 越接近 0 威胁越大，转换为 0~1 的强度
                damage_strength = (5 - turns_to_damage) / 5.0
            ...
            for x in range(MAP_SIZE):
                for y in range(MAP_SIZE):
                    ...
                    if effect.in_range(x, y):
                        ...
                        if storm_damage_channel is not None:
                            board[storm_damage_channel, x, y] = max(board[storm_damage_channel, x, y], damage_strength)
        return board
    ...
    def encode_stats(self, state: BackendState, player: int) -> np.ndarray:
        ...
        extras = np.array(
            [
                ...
                state.weapon_cooldowns[player, SuperWeaponType.LIGHTNING_STORM.value - 1]
                / SUPER_WEAPON_STATS[SuperWeaponType.LIGHTNING_STORM].cooldown,
                state.weapon_cooldowns[player, SuperWeaponType.EMP_BLASTER.value - 1]
                / SUPER_WEAPON_STATS[SuperWeaponType.EMP_BLASTER].cooldown,
                state.weapon_cooldowns[player, SuperWeaponType.DEFLECTOR.value - 1]
                / SUPER_WEAPON_STATS[SuperWeaponType.DEFLECTOR].cooldown,
                state.weapon_cooldowns[player, SuperWeaponType.EMERGENCY_EVASION.value - 1]
                / SUPER_WEAPON_STATS[SuperWeaponType.EMERGENCY_EVASION].cooldown,
                state.weapon_cooldowns[enemy, SuperWeaponType.LIGHTNING_STORM.value - 1]
                / SUPER_WEAPON_STATS[SuperWeaponType.LIGHTNING_STORM].cooldown,
                state.weapon_cooldowns[enemy, SuperWeaponType.EMP_BLASTER.value - 1]
                / SUPER_WEAPON_STATS[SuperWeaponType.EMP_BLASTER].cooldown,
                state.weapon_cooldowns[enemy, SuperWeaponType.DEFLECTOR.value - 1]
                / SUPER_WEAPON_STATS[SuperWeaponType.DEFLECTOR].cooldown,
                state.weapon_cooldowns[enemy, SuperWeaponType.EMERGENCY_EVASION.value - 1]
                / SUPER_WEAPON_STATS[SuperWeaponType.EMERGENCY_EVASION].cooldown,
