...
- `round_state.ants` 中的每条记录包含 `player`、`id`、`pos`、`hp`、`level`、`age`、`status`、`behavior`、`move`。
- `move = -1` 表示该蚂蚁本回合没有可记录的移动方向，例如刚生成。
- `round_state.towers` 不是“当前场上全部塔的完整快照”，而是“本回合发生变化的塔的增量记录”。
...
- 只有当 `Basic` 塔被 `13 id` 操作真正拆除时，回放里才会写出 `type = -1`。
